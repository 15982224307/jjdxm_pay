...
public class UPPayRSAUtil {
    ...
    public static PublicKey getPublicKeyPM() {
        ...
        String modulus = ConstantKeys.UpPay.PublicKeyPMModulus;
        String publicExponent = ConstantKeys.UpPay.publicExponent;
        PublicKey publicKey = UPPayRSAUtil.generateRSAPublicKey(modulus,
                publicExponent);
        return publicKey;
    }
    ...
    public static PublicKey getPublicKeyProduct() {
        ...
        String modulus = ConstantKeys.UpPay.PublicKeyProductModulus;
        String publicExponent = ConstantKeys.UpPay.publicExponent;
        PublicKey publicKey = UPPayRSAUtil.generateRSAPublicKey(modulus,
                publicExponent);
        return publicKey;
    ...
    public static boolean verify(String msg, String sign64, String mode) {
        ...
        try {
            if ("01".equals(mode)) {
                ret = UPPayRSAUtil.verifyPM(UPPayRSAUtil.sha1(msg.getBytes()).getBytes(),
                        Base64.decode(sign64, Base64.NO_WRAP));
            } else if ("00".equals(mode)) {
                ret = UPPayRSAUtil.verifyProduct(UPPayRSAUtil.sha1(msg.getBytes())
